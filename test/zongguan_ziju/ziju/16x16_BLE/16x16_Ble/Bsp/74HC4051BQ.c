...
#define ADC_DELAY_TIME (206 * 220) //9 ns * 120
...
void ReadAdcValue(Hc4051AdcValue *adc_raw_value)
{
    for(uint16_t AY = 0; AY < 4; ++AY)
    {
			AY_Selection(AY);
				...
				adc_raw_value->value[0][remake[AY]]      =  GetAdcValue(1);
				adc_raw_value->value[0][remake[AY] +  4] =  GetAdcValue(0);
				adc_raw_value->value[0][remake[AY] +  8] =  GetAdcValue(2);
				adc_raw_value->value[0][remake[AY] + 12] =  GetAdcValue(3);
