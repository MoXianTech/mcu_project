...
{	
			...
			for(uint8_t AY = 0; AY < 8; ++AY)
			{
				...
				/*32x64 ADC_Value*/	
				adc_raw_value->value[count][AY]      = GetAdcValue(6);
				adc_raw_value->value[count][AY +  8] = GetAdcValue(7);				
				adc_raw_value->value[count][AY + 16] = GetAdcValue(4);
				adc_raw_value->value[count][AY + 24] = GetAdcValue(5);			
				adc_raw_value->value[count][AY + 32] = GetAdcValue(3);
				adc_raw_value->value[count][AY + 40] = GetAdcValue(2);	
				adc_raw_value->value[count][AY + 48] = GetAdcValue(1);
				adc_raw_value->value[count][AY + 56] = GetAdcValue(0);			
			}		
}
...
{
	for(uint8_t ax = 0; ax < 25; ++ax)
	{
		for(uint8_t ay = 0; ay < 56; ++ay)
		{
			adc_raw_value->voltage_25x56_display[ax][ay] = adc_raw_value->voltage[ax][ay];
		}
	}
