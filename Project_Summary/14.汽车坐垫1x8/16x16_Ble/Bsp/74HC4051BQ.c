...
{
		uint8_t adc_rank[4] = {2,3,1,0};
    for(uint16_t AY = 0; AY < 4; ++AY)
    {
			AY_Selection(adc_rank[AY]);
//			for(uint16_t AX = 0; AX < AX_NUM; ++AX)
//			{
					AX_Selection(15);				
					Hc4051Delay(ADC_DELAY_TIME);	
					adc_raw_value->value[0][AY]      =  GetAdcValue(1);
					adc_raw_value->value[0][AY +  4] =  GetAdcValue(0);
					adc_raw_value->value[0][AY +  8] =  GetAdcValue(2);
					adc_raw_value->value[0][AY + 12] =  GetAdcValue(3);
//				}
    }
}
							...
							{	
								...
								{									
									adc_raw_value->voltage[AX][AY] 
										= ((uint32_t )adc_raw_value->value[AX][AY] * 255) / (Vref - (adc_raw_value->sumAX[AY] - adc_raw_value->value[AX][AY]));
								}
								else
								{
...
void display_reampping(Hc4051AdcValue *adc_raw_value)
{
		...
		{
			adc_raw_value->voltage_display[0][AY] = adc_raw_value->voltage[0][AY];
		}
		
		for(uint8_t AY = 5; AY < 8; ++AY)
		{
			adc_raw_value->voltage_display[0][AY] = adc_raw_value->voltage[0][AY + 1];
		}
