...
#define MAX_RING_BUF_SIZE       1300
#define USART_RX_CACHE_BUFFER_SIZE 1300
...
volatile uint8_t dma_ch1_idle = 1;
...
static rcu_periph_enum COM_CLK[COMn] = {EVAL_COM0_CLK, EVAL_COM1_CLK, EVAL_COM2_CLK};
static uint32_t COM_TX_PIN[COMn] = {EVAL_COM0_TX_PIN, EVAL_COM1_TX_PIN, EVAL_COM2_TX_PIN};
static uint32_t COM_RX_PIN[COMn] = {EVAL_COM0_RX_PIN, EVAL_COM1_RX_PIN, EVAL_COM2_RX_PIN};
static uint32_t COM_GPIO_PORT[COMn] = {EVAL_COM0_GPIO_PORT, EVAL_COM1_GPIO_PORT, EVAL_COM2_GPIO_PORT};
static rcu_periph_enum COM_GPIO_CLK[COMn] = {EVAL_COM0_GPIO_CLK, EVAL_COM1_GPIO_CLK, EVAL_COM2_GPIO_CLK};
...
//{
//    gd_eval_com_init(EVAL_COM1, 460800);
...
void nvic_config(uint32_t com)
{
    if (com == EVAL_COM0)
    {
        nvic_irq_enable(DMA0_Channel3_IRQn, 1, 0); //TX
        nvic_irq_enable(DMA0_Channel4_IRQn, 1, 1); //RX
    ...
    } else if (com == EVAL_COM2) {
				nvic_irq_enable(DMA0_Channel2_IRQn,1,1); //RX
				nvic_irq_enable(DMA0_Channel1_IRQn,1,0); //tX
...
uint8_t usart2_send_buffer(uint8_t *data, uint16_t len)
{
    CircBuf_Push(&USART2_RxCBuf, data, len);
    return 0;
}
/**
 * @brief
