...
void ProcessADCvalue(Hc4051AdcValue *adc_raw_value)
{
		uint16_t Vref = 4095;
		uint16_t flag_sum = 1;
		uint16_t display_threshold_value = 0;
		while(flag_sum)
		{					
							...
							{	
								...
								{									
									adc_raw_value->voltage[AX][AY] 
										= (adc_raw_value->value[AX][AY] * 255) / (Vref - (adc_raw_value->sumAX[AY] - adc_raw_value->value[AX][AY]));
//									adc_raw_value->d_voltage[AX][AY] 
//										= 1000000 / (Vref - adc_raw_value->voltage[AX][AY]) - 243;				
								}
								else
								{
...
void display_remaping(Hc4051AdcValue *adc_raw_value)
{
	for(uint8_t ax = 0; ax < 31; ++ax)
	{
		...
		{
			adc_raw_value->d_voltage_display[ax][50 - ay] = adc_raw_value->voltage[ax][ay + 13];	
		}
	}
