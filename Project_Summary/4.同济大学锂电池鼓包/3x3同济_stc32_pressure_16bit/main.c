  ...
  led_GPIO_config();
	...
	while(1)
	{
		...
		if(time_1ms % 10 == 0)
		{
			u8 ax = 0, ay = 0;
			u16 data_index = 0;
			u8 adc_rank[AY_NUM] = {2,4,6,1,3,7,0,5,8};
			...
			serial_frame[7] = 0x01;
				...
				{
					serial_frame[8 + data_index * 2] = adc_read.p_value[adc_rank[ay]] & 0x00ff;
					serial_frame[9 + data_index * 2] = adc_read.p_value[adc_rank[ay]] >> 8;
					data_index ++;
				}			
				data_index = 0;
