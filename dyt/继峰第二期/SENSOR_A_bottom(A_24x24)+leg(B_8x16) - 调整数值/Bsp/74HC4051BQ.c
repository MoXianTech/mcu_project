...
#define ADC_DELAY_TIME (9 * 220) //9 ns * 120
			...
			single_xl9555_io_set(ax_index);
					...
					for(uint8_t ay = 0; ay < 8; ++ay)
					{
						...
						if(ax_index < 24)
						{
								...
								adc_raw_value->sensorA_raw_value[ax_index][ay]	    = GetAdcValue(1) - 15;
								adc_raw_value->sensorA_raw_value[ax_index][ay +  8]	= GetAdcValue(0) - 15;
								adc_raw_value->sensorA_raw_value[ax_index][ay + 16]	= GetAdcValue(3) - 15;
						}
						else
						{
								Choose_AY_Channel(B_adc_rank[ay]);	
								Hc4051Delay(ADC_DELAY_TIME);
								adc_raw_value->sensorB_raw_value[ax_index - 24][ay]	      = GetAdcValue(5) + 25;
								adc_raw_value->sensorB_raw_value[ax_index - 24][ay +  8]	= GetAdcValue(4) + 25;
								adc_raw_value->sensorB_raw_value[ax_index - 24][ay + 16]	= GetAdcValue(2) + 25;
						}		
					}		
							...
							{			
									...
									{	
										...
										{									
											...
											adc_raw_value->sensorA_voltage[AX][AY] =  (adc_raw_value->sensorA_raw_value[AX][AY] * 255)
											/ (Vref - (adc_raw_value->sumAX[AY] - adc_raw_value->sensorA_raw_value[AX][AY]));	
											
											if(adc_raw_value->sensorA_voltage[AX][AY] > 255)
											{
												adc_raw_value->sensorA_voltage[AX][AY] = 254;
											}
										}
										else
										{
							...
							{			
									...
									{	
										...
										{									
											...
											if(adc_raw_value->sensorB_voltage[AX][AY] > 255)
											{
												adc_raw_value->sensorB_voltage[AX][AY] = 254;
											}
										}
										else
										{
...
void display_remapping(Hc4051AdcValue *adc_raw_value)
{
	...
	{
		for(uint8_t ay = 0; ay < 20; ++ay)
		{
			adc_raw_value->sensorB_display[ax][ay + 4] = adc_raw_value->sensorB_voltage[ax + 4][ay];
		}
	}
	...
	for(uint8_t ax = 0; ax < AX_NUM; ++ax)
	{
		for(uint8_t ay = 0; ay < AY_NUM; ++ay )
		{
			adc_raw_value->sensorA_display[ay][ax] = adc_raw_value->sensorA_voltage[ax][ay];
		}
	}
