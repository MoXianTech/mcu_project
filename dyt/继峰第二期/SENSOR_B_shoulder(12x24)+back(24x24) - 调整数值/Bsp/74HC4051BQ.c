			...
			uint8_t B_adc_rank[8] = {4,6,7,5,2,1,0,3};
					...
					for(uint8_t ay = 0; ay < 8; ++ay)
					{
						...
						if(ax_index < 24)
						{
								...
								adc_raw_value->sensorA_raw_value[ax_index][ay]	    = GetAdcValue(1) + 20;
								adc_raw_value->sensorA_raw_value[ax_index][ay +  8]	= GetAdcValue(0) + 20;
								adc_raw_value->sensorA_raw_value[ax_index][ay + 16]	= GetAdcValue(3) + 20;
						}
						else
						{
								Choose_AY_Channel(B_adc_rank[ay]);	
								Hc4051Delay(ADC_DELAY_TIME);
								adc_raw_value->sensorB_raw_value[ax_index - 24][ay]	      = GetAdcValue(5) - 15;
								adc_raw_value->sensorB_raw_value[ax_index - 24][ay +  8]	= GetAdcValue(4) - 15;
								adc_raw_value->sensorB_raw_value[ax_index - 24][ay + 16]	= GetAdcValue(2) - 15;
						}		
					}		
