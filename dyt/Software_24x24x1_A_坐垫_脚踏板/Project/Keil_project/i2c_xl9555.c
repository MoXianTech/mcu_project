...
void single_xl9555_io_reset(uint8_t io_num)
{
    i2c_write_data(I2C_COM0, xl9555_rank[io_num / 8 - 3], END_IO_REG, 0x00 << (io_num % 8));
}

void single_xl9555_io_set(uint8_t io_num)
{
		if (io_num < 24)
        i2c_write_data(I2C_COM0, xl9555_rank[2 - io_num / 8], START_IO_REG, 0x01 << (io_num % 8)); //0x80 >> (io_num % 8)
    else
...
{
    if (io_num < 24)
        i2c_write_data(I2C_COM0, xl9555_rank[2  - io_num / 8], START_IO_REG, 0x00);
    else
