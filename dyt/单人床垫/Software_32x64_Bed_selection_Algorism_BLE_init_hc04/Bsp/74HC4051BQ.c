...
{	
			...
			for(int8_t AY = 0; AY < 8; ++AY)
			{
					...
					{							
						...
						/*32x64 ADC_Value*/	
						adc_raw_value->value[AX][AY]      = GetAdcValue(6) / 2;
						adc_raw_value->value[AX][AY +  8] = GetAdcValue(7) / 2;				
						adc_raw_value->value[AX][AY + 16] = GetAdcValue(4) / 2;
						adc_raw_value->value[AX][AY + 24] = GetAdcValue(5) / 2;			
						adc_raw_value->value[AX][AY + 32] = GetAdcValue(3) / 2;
						adc_raw_value->value[AX][AY + 40] = GetAdcValue(2) / 2;	
						adc_raw_value->value[AX][AY + 48] = GetAdcValue(1) / 2;
						adc_raw_value->value[AX][AY + 56] = GetAdcValue(0) / 2;			
				}
			}		
}
...
void ProcessADCvalue(Hc4051AdcValue *adc_raw_value)
{
		uint16_t Vref = 4096;
		uint16_t flag_sum = 1;
		uint16_t display_threshold_value = 1;
		while(flag_sum)
		{					
