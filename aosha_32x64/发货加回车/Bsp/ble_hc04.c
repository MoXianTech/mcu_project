...
void ble_hc04_init(USART_COM_ID_T com)
{
    ...
    //at_config_at_cmd_timeout(com, "AT+NAME=MX_SENSOR", NULL, 0, "", 500);
    at_config_at_cmd_timeout(com, "AT+BNAME=MX_BLE_SENSOR_L11", NULL, 0, "", 500);
...
void ble_hc04_thread_loop(void *handle)
{
    ...
    if (BLE_LINK_GET == 0)
    {
        ...
        process_handle->upload_interval = 200;
    } else {
