...
void usart_init(uint32_t baudrate, uart_port_t uart_port)
{
    ...
    /* 配置uart引脚 */
    uart_set_pin(uart_port, USART_TX_GPIO_PIN, USART_RX_GPIO_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);

    /* 安装串口驱动 */
    uart_driver_install(uart_port, RX_BUF_SIZE * 2, RX_BUF_SIZE * 2, 20, NULL, 0);
}
