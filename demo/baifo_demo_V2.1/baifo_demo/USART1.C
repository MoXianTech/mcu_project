...
COMx_Define	COM1;
u8	idata TX1_Buffer[COM_TX1_Lenth] = { 0 };	//发送缓冲
u8 	idata RX1_Buffer[COM_RX1_Lenth] = { 0 };	//接收缓冲
...
void Uart1SendData(u8 *puts, u8 len)
{
    int i = 0;
	for(i = 0; i < len; ++i)
    {
...
#if 0
int fputc(char ch, v *f)
{
	TX1_write2buff((char)ch);
    return ch;
}
#endif
