...
static void LCD_gpio_config(void)
{
    ...
    gpio_mode_set(DC_RST_CS_GPIO_Port, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE,  DC_Pin);
    gpio_output_options_set(DC_RST_CS_GPIO_Port, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, DC_Pin);
...
void LCD_init(void)
{
    ...
    Paint_DrawImage(gImage_image, 72, 0, 95, 280);
    //Paint_DrawImage(gImage_image, 0, 0, 240, 280);
    delay_1ms(2000);
...
uint16_t ADC_value_transition_color(uint8_t value)
{
    ...
    else
    {
        ...
        tmep =  ((uint32_t)(value - min_value) * 94 * 10 / (65 - min_value) + 5) / 10;
    }
...
static uint16_t spi_dma_buffer[25920] = {0};
void LCD_ADC_DMA_display(uint8_t * adc_value)
{
    uint8_t j = 0,l = 0;
    uint16_t Color = 0;
    uint16_t spi_Color = 0;
    uint32_t temp_cnt = 0;
    static uint8_t count = 0;

    if(count == 0)
    {
        LCD_1IN69_SetWindows(12, 20, 228, 260);//设置屏幕adc采样显示范围
        ...
        for(count = 0; count < 40; count++)
            for(l = 0; l < PIXEL_HEIGHT;l++)
                for(j = 0; j < POS_X; j++)
                {
                    ...
                    spi_Color = ((Color & 0xff00)>>8) | ((Color & 0x00ff)<<8);
                    spi_dma_buffer[temp_cnt] = spi_Color;
                    spi_dma_buffer[temp_cnt+1] = spi_Color;
                    spi_dma_buffer[temp_cnt+2] = spi_Color;
    ...
    spi3_send_dma_buffer((uint8_t*)spi_dma_buffer,51840);
