...
void power_save_thread(void)
{
    ...
    power_save.moto_pos_info[MOTO_A] = get_encoder_currentCount(MOTO_A);
    power_save.moto_pos_info[MOTO_B] = get_encoder_currentCount(MOTO_B);
    power_save.moto_pos_info[MOTO_B] = get_encoder_currentCount(MOTO_C);
    power_save.moto_pos_info[MOTO_C] = get_encoder_currentCount(MOTO_D);

    fmc_write_8bit_data(WRITE_ADDRESS_BYTE, sizeof(power_save), (int8_t *)&power_save);

    printf("%s %d %d %d %d", __FUNCTION__,
