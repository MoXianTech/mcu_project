...
void adc_value_read(process_handle_t *process_handle, uint8_t count_y, ADC_MODE_TYPE_T mode_type)
{
    ...
    switch (mode_type)
    {
		...
		sprintf((char *)process_handle->printf_buffer, "%d %d %d %d %d %d %d %d\n", process_handle->adc_raw_value[0][63],
																													 process_handle->adc_raw_value[1][63],
		process_handle->adc_raw_value[2][63],
		process_handle->adc_raw_value[3][63],
		process_handle->adc_raw_value[4][63],
		process_handle->adc_raw_value[5][63],
		process_handle->adc_raw_value[6][63],
		process_handle->adc_raw_value[7][63]
		);
}

void adc_value_read_single(process_handle_t *process_handle)
{
		uint8_t count = 1;
		...
		for (count = 0; count < 8; count ++)
		{
				select_x_control(count);
				Hc4051Delay(ADC_DELAY_TIME);
				process_handle->adc_raw_value[0][count] = GetAdcValue(adc_rank[count]) / 2;
		}
