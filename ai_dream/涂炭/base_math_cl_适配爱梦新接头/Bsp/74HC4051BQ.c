...
void display_remapping(process_handle_t *process_handle)
{

    uint8_t count_x = 0, count_y = 0;
	
    for(count_x = 0; count_x < 29; count_x ++)
    {

        memcpy(process_handle->adc_remap_array[count_x],
               process_handle->adc_cali_value[count_x + 3] + 4,
               28);
    }
}
