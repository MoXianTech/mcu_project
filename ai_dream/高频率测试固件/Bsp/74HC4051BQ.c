        ...
        case TOTAL_VCC_READ:
            ...
            break;
				case ONCE_VCC_READ:
					  for (count_1 = 0; count_1 < 8; count_1 ++)
            {
							process_handle->adc_cali_value[0][count_1] = GetAdcValue(adc_rank[count_1]) / 16;
            }
						break;
        default:
            break;
...
void select_x_control(uint8_t x_value)
{
    ...
    x_default = gpio_output_port_get(GPIOB);
    ...
    x_default |= (hardware_pint << 12);

    gpio_port_write(GPIOB, x_default);
