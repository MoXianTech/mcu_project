...
void drawText(int colorWheelOffset)
{
  ...
  dma_display->setCursor(5, 16);    // start at top left, with 8 pixel of spacing
  uint8_t w = 0;
  const char *str = "WaveshareElectronics";
  for (w=0; w<9; w++) {
    dma_display->setTextColor(colorWheel((w*32)+colorWheelOffset));
    dma_display->print(str[w]);
  }
  dma_display->setCursor(0, 24);    // next line
  for (w=9; w<20; w++) {
    dma_display->setTextColor(colorWheel((w*32)+colorWheelOffset));
    dma_display->print(str[w]);
  }
